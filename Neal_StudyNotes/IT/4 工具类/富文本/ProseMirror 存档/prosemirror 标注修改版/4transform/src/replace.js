...
//
// > 将一个 slice 「恰当的」放到文档中指定的位置，会生成一个进行插入操作的 step。如果没有一个有意义的途径来插入该 slice，或者插入的 slice
...
// given `slice`.
//
// > 用指定的 `slice` 替换在 `from` 和 `to` 之间的这部分文档。
...
//
// > 用指定的内容替换指定过的 range，该内容可能是一个 fragment、节点、或者节点数组。
...
//
// > 用指定的 slice 替换文档的一个 range，用 `from`、`to` 以及 slice 的 [`openStart`](#model.Slice.openStart) 属性
...
// that parent node.
//
// > 用指定的 node 替换一个由指定 `from` 和 `to` 模糊确定的 range，而不是一个精确的位置。
// 如果当 from 和 to 相同且都位于父级节点的起始或者结尾位置，而指定的 node 并不适合此位置的时候，该方法可能将 from 和 to 的范围 _扩大_ 到
// 超出父级节点以允许指定的 node 被放置，如果指定的 range（from 和 to 形成的）完全覆盖了一个父级节点，则该方法可能完全替换掉这个父级节点。
...
//
// > 删除指定的 range，会将该 range 扩大到完全覆盖父级节点，直到找到一个有效的替换为止。
//
