...
//
// > 默认情况下，文档节点使用它们 schema 配置对象中的 [`toDOM`](#model.NodeSpec.toDOM)
...
//
// > Mark 的 view 仅仅支持 `dom` 和 `contentDOM`，而且不支持任何 node view 的方法。
//
...
//
//   > 应该持有节点的内容的 DOM 节点。只有当定义了 `dom` 属性且节点类型不是叶子节点的时候才有意义。
...
//   ProseMirror.
//
//   > 当 node view 更新自身的时候会调用该节点的此方法。它接受一个 node（可能是与当前不同的类型）、
...
//
//   > 该方法将会被调用以用来处理节点内部选区的设置。`anchor` 和 `head` 位置相对于节点的起始位置。
...
//   ignored.
//
//   > 当一个 DOM [mutation（突变）](https://developer.mozilla.org/en-US/docs/Web/API/MutationObserver)
...
//   editor is destroyed.
//   
//   > 当整个编辑器被销毁或者当前 node view 被移除的时候调用。
