...
#1.为避免添加复数形式's 需要，在上面设定数据表名字,{ collection: 'user' }
  // 创建集合(数据表) 并设定数据表名字Course；实际会是小写+s的courses
 const Course = mongoose.model('Course', courseSchema); 
// 创建一条数据 ,不写入数据的情况，服务器并没有真实的创建对应的数据库
...
course.save()
 ...
 .catch(err => console.log('数据写入失败' + err));
~~~
...
//方式 2 在函数返回值中获取写入回调
Course.create({name: '马三', age: 25, email: "macsan@abcd.cn"}, (err, doc) => { 
     //  错误对象
    console.log(err)
     //  当前插入的文档
    console.log(doc)
...
~~~js
##注意mongoose.model方法中第三个参数不管，会自动添加复数形式s
