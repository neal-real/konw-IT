...
// > 一个有序列表的 [节点配置对象](#model.NodeSpec)。有一个唯一的属性 `order`，它决定了列表从哪个数字开始计数，默认是 1。
// 表现形式是一个 `<ol>` 元素。
...
// > 为 schema 方便的添加列表相关的节点类型到一个特定的节点类型的函数。[`orderedList`](#schema-list.orderedList) 表示为 `"ordered_list"`，

// > `itemContent` 决定了列表项的内容表达式。如果你想要在本模块中定义的命令应用于你自己的列表结构，则它的值应该是诸如 `"paragraph block*"`
...
// > 返回一个命令函数，该函数用给定的类型和属性包裹位于 list 中的选区。如果 `dispatch` 参数是 null，则只返回一个指示该行为是否可能的值，
// 而不实际执行修改。
