...
//
// > 该类的实例表示一个节点类型的 [content expression（内容表达式）](#model.NodeSpec.content) 的匹配状态，
...
export class ContentMatch {
  ...
  // content expression.
  //
  // > 尝试匹配指定的 fragment，如果失败，则会查看是否可以通过在该 fragment 前面插入一些节点来使之匹配。
  ...
  // exists.
  //
  // > 寻找一个包裹指定节点的节点集合，该集合中的节点在包裹住指定类型的节点后才能出现在当前位置。
  // 集合的内容可能是空（如果指定类型节点直接就适合当前位置而无需包裹任何节点时），若不存在相应的包裹节点，则集合也可能是 null。
  findWrapping(target) {
  ...
  // automaton that describes the content expression.
  //
  // > 在描述内容表达式的有限自动机中获取该节点第 _n_ 个外部的边界。
  edge(n) {
    let i = n << 1
